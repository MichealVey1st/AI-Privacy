...
import whisper # For speech recognition and transcription
import pyaudio # For audio input/output
import threading # For threading to handle audio input/output
import webrtcvad # For voice activity detection
from pydub import AudioSegment # for audio isolation
...
def distort_audio(input_audio, output_audio):
    # Load the mono audio for processing
    audio = AudioSegment.from_file(input_audio, format="wav")

    # Apply distortion effects
    distorted_audio = (
        audio
        .low_pass_filter(8000)  # Remove high frequencies
        .high_pass_filter(300)  # Remove low frequencies
        .apply_gain(-5)         # Reduce volume
    )

    # Shift pitch (increase or decrease)
    pitch_semitones = -5  # Negative for lower pitch, positive for higher
    distorted_audio = distorted_audio._spawn(
        distorted_audio.raw_data,
        overrides={
            "frame_rate": int(distorted_audio.frame_rate * (2.0 ** (pitch_semitones / 12.0)))
        }
    ).set_frame_rate(audio.frame_rate)

    # Export the distorted audio to a new file
    distorted_audio.export(output_audio, format="wav")
    print(f"Distorted audio saved to {output_audio}")


def modify_voices(video_path, output_path):
    # Step 1: Extract audio from the video
    audio_output_path = os.path.abspath("./temp/audio_temp_video.wav")
    distorted_audio_path = os.path.abspath("./temp/audio_distorted.wav")
    
    command = [
        "ffmpeg",
        "-i", video_path,        # Input video
        "-vn",                   # Extract audio
        "-acodec", "pcm_s16le",  # Save as uncompressed WAV
    ...
    # Step 2: Apply distortion to the audio
    distort_audio(audio_output_path, distorted_audio_path)

    # Step 3: Merge distorted audio back into the video
    command_merge = [
        "ffmpeg",
        "-i", video_path,           # Input original video
        "-i", distorted_audio_path, # Input distorted audio
        "-c:v", "copy",             # Copy video codec (no re-encoding)
        "-c:a", "aac",              # Re-encode audio as AAC
        "-map", "0:v:0",            # Map video stream from original video
        "-map", "1:a:0",            # Map audio stream from distorted audio
        output_path
    ]
